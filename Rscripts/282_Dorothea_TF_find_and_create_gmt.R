...
Dorothea_and_gmt = function(option_list)
{
  ...
  indx.int<-grep(paste(TF_genes_NO_NA, collapse='|'), net.df$source)
  ...
  net.df_oI<-net.df[which(net.df$source%in%TF_genes_NO_NA),]
  ...
  for(i in 1:length(Symbol_array)){
    ...
    for_gmt_net.df_oI_NO_NA$name[indx.Symbol]<-paste('Dorothea_ABCD',Symbol_array_sel,'targets', sep="_")
 ...
 ### save as gmt ----
  ...
  writeGMT(for_gmt_net.df_oI_NO_NA_reordered, paste("Dorothea_ABCD_",paste(Symbol_array, collapse="_"),'.gmt',sep=''))
...
Dorothea_and_gmt_specific = function(option_list)
{
  ...
  indx.int<-grep(paste(selected_TFs, collapse='|'), net.df$source)
  ...
  net.df_oI<-net.df[which(net.df$source%in%selected_TFs),]
