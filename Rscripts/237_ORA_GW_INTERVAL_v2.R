...
AP_function = function(option_list)
{
  ...
  #### READ and transform VAR_array_sel ----
  
  VAR_array_sel = opt$VAR_array_sel
  
  cat("VAR_array_sel_\n")
  cat(sprintf(as.character(VAR_array_sel)))
  cat("\n")
  
  #### READ ALL_by_ALL_INTERVAL_subset_data_wrangled ----
  
 
  ALL_by_ALL_INTERVAL_subset_data_wrangled<-readRDS(file=opt$ALL_by_ALL_INTERVAL_subset_data_wrangled)
  
  cat("ALL_by_ALL_INTERVAL_subset_data_wrangled_0\n")
  cat(str(ALL_by_ALL_INTERVAL_subset_data_wrangled))
  cat("\n")
  cat(str(unique(ALL_by_ALL_INTERVAL_subset_data_wrangled$VAR)))
  cat("\n")
 
  ALL_by_ALL_INTERVAL_subset_data_wrangled_subset<-ALL_by_ALL_INTERVAL_subset_data_wrangled[which(ALL_by_ALL_INTERVAL_subset_data_wrangled$VAR == VAR_array_sel),]
  
  ALL_by_ALL_INTERVAL_subset_data_wrangled_subset<-ALL_by_ALL_INTERVAL_subset_data_wrangled_subset[order(ALL_by_ALL_INTERVAL_subset_data_wrangled_subset$Genome_wide_pvalue, decreasing=F),]
  
  cat("ALL_by_ALL_INTERVAL_subset_data_wrangled_subset_0\n")
  cat(str(ALL_by_ALL_INTERVAL_subset_data_wrangled_subset))
  cat("\n")
  cat(str(unique(ALL_by_ALL_INTERVAL_subset_data_wrangled_subset$VAR)))
  cat("\n")
  
  
  ALL_by_ALL_INTERVAL_subset_data_wrangled_subset$ENTREZID <- mapIds(org.Hs.eg.db, keys=ALL_by_ALL_INTERVAL_subset_data_wrangled_subset$HGNC, keytype="SYMBOL",
                                 column="ENTREZID")
  
  cat("ALL_by_ALL_INTERVAL_subset_data_wrangled_subset_1\n")
  cat(str(ALL_by_ALL_INTERVAL_subset_data_wrangled_subset))
  cat("\n")
  
  
  ALL_by_ALL_INTERVAL_subset_data_wrangled_subset_NO_NA<-ALL_by_ALL_INTERVAL_subset_data_wrangled_subset[!is.na(ALL_by_ALL_INTERVAL_subset_data_wrangled_subset$ENTREZID),]
  
  cat("ALL_by_ALL_INTERVAL_subset_data_wrangled_subset_NO_NA_0\n")
  cat(str(ALL_by_ALL_INTERVAL_subset_data_wrangled_subset_NO_NA))
  cat("\n")
  
  
  
 
  indx.DUP<-which(duplicated(ALL_by_ALL_INTERVAL_subset_data_wrangled_subset_NO_NA$ENTREZID) == TRUE)
  ...
  if(length(indx.DUP) >0)
  {
    ...
    ALL_by_ALL_INTERVAL_subset_data_wrangled_subset_NO_NA_NO_DUP<-ALL_by_ALL_INTERVAL_subset_data_wrangled_subset_NO_NA[-indx.DUP,]
    
  }else{
    
    ALL_by_ALL_INTERVAL_subset_data_wrangled_subset_NO_NA_NO_DUP<-ALL_by_ALL_INTERVAL_subset_data_wrangled_subset_NO_NA
    
  }#length(indx.DUP) >0
  
  
  cat("ALL_by_ALL_INTERVAL_subset_data_wrangled_subset_NO_NA_NO_DUP_0\n")
  cat(str(ALL_by_ALL_INTERVAL_subset_data_wrangled_subset_NO_NA_NO_DUP))
  cat("\n")

 
  if(dim(ALL_by_ALL_INTERVAL_subset_data_wrangled_subset_NO_NA_NO_DUP)[1] >0)
  {
    
    indx.rest<-c(which(colnames(ALL_by_ALL_INTERVAL_subset_data_wrangled_subset_NO_NA_NO_DUP) == 'HGNC'),
                 which(colnames(ALL_by_ALL_INTERVAL_subset_data_wrangled_subset_NO_NA_NO_DUP) == 'ENTREZID'),
                 which(colnames(ALL_by_ALL_INTERVAL_subset_data_wrangled_subset_NO_NA_NO_DUP) == 'VAR'),
                 which(colnames(ALL_by_ALL_INTERVAL_subset_data_wrangled_subset_NO_NA_NO_DUP) == 'Genome_wide_pvalue'))
    ...
    scores<-matrix(ALL_by_ALL_INTERVAL_subset_data_wrangled_subset_NO_NA_NO_DUP$Genome_wide_pvalue)
    row.names(scores)<-ALL_by_ALL_INTERVAL_subset_data_wrangled_subset_NO_NA_NO_DUP$ENTREZID
    colnames(scores)<-"Genome_wide_pvalue"
      ...
      if(length(gmt_sel_GREP) >0)
      {
        ...
        for(iteration_gmt_sel_GREP in 1:length(gmt_sel_GREP))
        {
          ...
          if(dim(reference_gmt_sel)[1] >= minGS_size & dim(reference_gmt_sel)[1] <= maxGS_size)
          {
            ...
            indx<-which(unique(reference_gmt_sel$id)%in%c("CUX1","FOXP1","GATA2","GFI1"))
            ...
            if(FLAG_oversized_but_interesting > 0)
            {
              ...
              cat(sprintf(as.character(unique(reference_gmt_sel$id))))
              cat("\n")
        ...
        # cat("gmt_sel_GREP[indx.select]\n")
        # cat(str(gmt_sel_GREP[indx.select]))
        # cat("\n")
        
        
        AP_result<-ActivePathways(scores, gmt_sel_GREP[indx.select], background= background,
                                  cutoff = 0.1,
                                  significant = pval_threshold)
        ...
        if(FLAG_null == 0)
        {
          ...
          if(dim(AP_result_df)[1] >0)
          {
            ...
            AP_result_df$HGNC<-NA
            
            AP_result_df$HGNC <- mapIds(org.Hs.eg.db, keys=AP_result_df$ENTREZ, keytype="ENTREZID",
            ...
            AP_result_df_collapsed<-as.data.frame(AP_result_df.dt[,.(n_genes_in_overlap=length(ensembl_gene_id),
                                                                     overlap_HGNC=paste(HGNC, collapse='|'),
    ...
    if(length(Final_result)>0)
    {
      ...
      FINAL_df_MAX$VAR_array_sel<-VAR_array_sel
      ...
      indx.site<-grep("https:", ALL_tested)
      
      cat("indx.site_0\n")
      cat(str(indx.site))
      cat("\n")
      
      ALL_tested_sel<-ALL_tested[indx.site]
      
      cat("ALL_tested_sel_0\n")
      cat(str(ALL_tested_sel))
      cat("\n")
      
      ALL_tested_sel.df<-as.data.frame(ALL_tested_sel)
      ...
      write.table(ALL_tested_sel.df,file=paste("ALL_tested_",VAR_array_sel,".tsv", sep=''),sep="\t", quote=F, row.names = F)


      write.table(FINAL_df_MAX,file=paste("results_",VAR_array_sel,".tsv", sep=''),sep="\t", quote=F, row.names = F)
  ...
  }#dim(ALL_by_ALL_INTERVAL_subset_data_wrangled_subset_subset_NO_NA_NO_DUP)[1] >0
}
...
main = function() {
  ...
  option_list <- list(
    ...
    make_option(c("--maxGS_size"), type="numeric", default=NULL, 
                metavar="type", 
                help="Path to tab-separated input file listing regions to analyze. Required."),
    make_option(c("--ALL_by_ALL_INTERVAL_subset_data_wrangled"), type="character", default=NULL, 
                metavar="type", 
                help="Path to tab-separated input file listing regions to analyze. Required."),
    make_option(c("--background_genes"), type="character", default=NULL, 
                metavar="type", 
                help="Path to tab-separated input file listing regions to analyze. Required."),
    make_option(c("--VAR_array_sel"), type="character", default=NULL, 
                metavar="filename", 
                help="Path to tab-separated input file listing regions to analyze. Required."),
